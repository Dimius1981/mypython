    ...
    def __add__(self, other):
        ...
        if ((self.__my_cols == other.__my_cols) and
           (self.__my_rows == other.__my_rows)):
            for row in range(self.__my_rows):
                __row_list = list()
                for col in range(self.__my_cols):
                    __row_list.append(self.__my_list[row][col] + other.__my_list[row][col])
                __sum_list.append(__row_list.copy())
                __row_list.clear()
            return Matrix(__sum_list)
        else:
